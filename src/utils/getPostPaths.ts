...
function getCompatibilityPostSlugPath(
  id: string,
  filePath: string | undefined
): string {
  const pathSegments = getPostPathSegments(filePath).map(segment =>
    slugifyStr(segment)
  );
  const slug = getIdSlug(id);
  return pathSegments.length > 0
 ...
 * Directory casing is preserved because these are the historical canonical
 * URLs that search engines already discovered.
 * e.g. `/ai/XGEN/my-post`
 */
 ...
 * e.g. `/ai/XGEN/my-post` or `/en/ai/XGEN/my-post`
 */
...
/** Returns the temporary `/posts/...` route kept for inbound compatibility. */
export function getCompatibilityPostSlug(
  id: string,
  filePath: string | undefined
): string {
  return `/${getCompatibilityPostSlugPath(id, filePath)}`;
}
