...
## 첫 번째 버전은 그럴듯했지만 쓸 수 없었다

처음 만든 리포트는 겉으로 보기에는 성공처럼 보였다. 영상 하나를 넣으면 위반 의심 항목이 나오고, 각 항목마다 점수와 설명도 붙었다. 하지만 실제로 읽어 보니 바로 실무에 쓰기 어려웠다.

- 같은 고지 문구가 프레임마다 조금씩 다르게 읽혀 여러 건으로 잡혔다.
- 상품 유형과 무관한 필수 문구가 빠졌다고 표시됐다.
- 규정 원문보다 과거 사례의 말투에 끌려간 판정이 있었다.
- 모델 점수는 높지만 화면상 의미 없는 이미지 후보가 올라왔다.
- "위반 의심"이라고만 쓰고, 어느 장면을 다시 봐야 하는지 설명이 부족했다.

이때 방향을 바꿨다. 목표는 위반을 많이 찾는 것이 아니라, 사람이 다시 봐야 할 장면을 줄이는 것이다. 그래서 이후 작업은 전부 리포트 건수를 줄이고, 각 항목에 시간, 위치, 규정 원문, 유사 사례 근거를 붙이는 방향으로 흘러갔다.
...
당황스러웠던 점은 결과가 허술하게 틀리는 것이 아니라, 매우 그럴듯하게 틀린다는 점이었다. LLM은 유사 사례를 붙여서 자신 있게 설명했고, 문장만 보면 납득되는 경우도 있었다. 문제는 그 설명이 실제 규정의 적용 조건을 건너뛸 때가 있다는 것이었다.
...
처음에는 많이 잡는 쪽이 더 안전하다고 생각했다. 그런데 실제 리포트를 펼쳐 보면 생각이 바뀐다. 오탐이 너무 많으면 사람은 결국 모든 항목을 의심하게 되고, 그러면 자동화가 시간을 아껴 주지 못한다. 좋은 심의 보조 도구는 "가능한 모든 의심"을 던지는 도구가 아니라, "지금 먼저 봐야 할 의심"을 정리하는 도구에 가까웠다.
