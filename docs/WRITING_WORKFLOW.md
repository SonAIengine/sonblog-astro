...
9. 프로덕션 URL, canonical, sitemap, `llms.txt`를 확인하고 Search Console의 크롤링을 기다린다.
...
- 대표 이미지가 필요하면 실제 화면, 구조도, 결과 그래프처럼 글의 내용을 설명하는 이미지를 쓴다. 모든 글에 같은 로고 이미지만 반복하지 않는다.
- `canonicalURL`은 외부에 원문이 있거나 명시적인 대표 URL이 필요한 경우에만 설정한다.
- 제목이나 본문에 같은 검색어를 부자연스럽게 반복하지 않는다.

검색엔진과 AI 검색 노출을 위한 사이트 운영 절차는 `docs/DISCOVERABILITY.md`를 따른다. `llms.txt`는 보조 탐색 문서이며 Google 색인이나 순위를 보장하지 않는다.
